      ...
      <uploadComponent></uploadComponent>  
        ...
        <div
          ...
          >
            ...
            <div
              v-if="msg.contentType == 'tongue'"
              class="bubble bubb assistant-bubble last-message"
            >
              <div id="medical-page" class="medical-box face-box">
                <div class="img-box">
                  <img width="200px" :src="tongueDiagnosis.imgUrl" alt="" />
                </div>
                <div
                  class="desc-box"
                  v-html="renderMessage(tongueDiagnosis.recommendation)"
                ></div>
              </div>
            </div>
        ...
        <div
          ...
          <div
            ...
            style="
              ...
              align-items: center;
            "
            v-if="afterLoading && msg.contentType == 'tongue'"
            >
            <div class="avatar" style="align-self: flex-start">
              <img
                src="@/assets/chat_pictures/icon.png"
...
const gettongueByImage = (url) => {
  isLoading.value = true;
  store.commit("setIsLoading", true);
  // url =
  //   "https://jingfang-images-1322234581.cos.ap-beijing.myqcloud.com/tongues%2F1710841951364tz.png";
  axiosPost(`/tongue/diagnosis?img=${url}`).then((response) => {
    console.log(
      "response.data.tongueDiagnosis",
      response.data.data.tongueDiagnosis,
     
    );
    const tongueDiagnosisnoimg = {
      ...response.data.data.tongueDiagnosis,
      imgUrl: undefined,
    };
    tongueDiagnosis.value = tongueDiagnosisnoimg;
    console.log(tongueDiagnosis.value)
    messages.push({
      chatId: chatId.value,
      content: "",
      contentType: "tongue",
...
const isnotice = computed(() => {  
      if (store.state.SheTai) {  
        
        return store.state.SheTai.isnotice;  
      } else {  
        console.error('tongue module is not defined in the store state');  
        return false;  
      }  
    });  
